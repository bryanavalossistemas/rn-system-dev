...
export default function CreateForm({ setOpen, suppliers }: CreateFormProps) {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onMutate: async ({ formData }) => {
      ...
      const item: Purchase & {
        ...
        document: {
          total: documentDetails.reduce((total, d) => total + parseFloat(d.unitPrice) * parseInt(d.quantity), 0),
      ...
      queryClient.setQueryData(['purchases', dateOption], (oldItems: Purchase[]) => {
  ...
  const [currentSuppliers, setCurrentSuppliers] = useState(suppliers.filter((s) => s.type === (currentDocumentTypeId === '1' ? 'RUC' : 'DNI')));
                  ...
                  <TableFooter>
                    <TableRow>
                      ...
                      <TableCell className="text-right">
                        {formatCurrency(
                          field.value.reduce((acummulator, detail) => acummulator + Number(detail.unitPrice) * Number(detail.quantity), 0),
                        )}
                      </TableCell>
                    </TableRow>
                  </TableFooter>
